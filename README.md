...
一个面向多语言文案交付的 AI 工作流作品集项目。公开版只展示问题拆解、系统架构、评测方法和可运行的匿名化示例，不包含任何真实业务数据、项目代号、内网系统、接口细节或生产记录。

## 项目概览

多语言交付经常卡在三个地方：需求输入不规范、翻译质量难量化、上线前格式校验依赖人工。这个项目把流程拆成 `Plan -> Act -> Eval -> Delivery` 四段，用可审计的规则校验和评测闭环降低返工。

核心能力：

- 自动解析需求行，识别缺失 key、占位符、字符限制和目标语种
- 缺 key 时生成命名候选，并通过规则校验约束格式
- 将术语召回、翻译、文化适配和格式归一化拆成可替换 worker
- 用规则校验覆盖占位符、key 格式、长度限制和双端一致性
- 将低分样本回流到术语表、prompt 和 golden set
- 通过交付适配层对接内部平台，公开版仅保留接口边界，不暴露真实实现
...
更完整的系统图和评测闭环见 [docs/architecture.md](docs/architecture.md)。
